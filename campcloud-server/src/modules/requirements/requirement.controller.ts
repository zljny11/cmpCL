...
export class RequirementsController {
  ...
  @Post(':id/dataset-batches/commit-oss-files')
  async createDatasetBatchFromOssFiles(
    @CurrentUser() user: AuthUser,
    @Req() request: Request,
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: CreateDatasetBatchFromOssFilesDto,
  ) {
    const result = await this.requirementsService.prepareDatasetBatchFromOssFiles(
      BigInt(user.id),
      BigInt(id),
      user.role,
      dto,
  ...
  @Post(':id/pull-detail-data')
  async pullRequirementDetailData(
    @CurrentUser() user: AuthUser,
    @Req() request: Request,
    @Param('id', ParseIntPipe) id: number,
  ) {
    const result = await this.requirementsService.pullRequirementDetailData(BigInt(user.id), BigInt(id), user.role);
    await this.adminLogsService.createLog({
      actor: user,
      category: AdminOperationLogCategory.data,
      action: '拉取需求详情数据',
      ...
      targetId: id.toString(),
      detail: {
        startedBatchIds: result.startedBatchIds,
        skippedBatchIds: result.skippedBatchIds,
